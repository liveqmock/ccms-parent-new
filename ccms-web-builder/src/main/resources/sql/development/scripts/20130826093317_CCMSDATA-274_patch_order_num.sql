...
BEGIN
	...
	-- . 从增量表补订单num
    UPDATE plt_taobao_order i , (select tid,num,modified from top_trade_incr_job_buffer where num is not null and job_status = 'DOING' order by modified DESC) t  
    SET i.num =t.num
    WHERE i.tid = t.tid
	 AND i.modified <= t.modified;
    ...
    -- 8. 更新订单评价
     REPLACE INTO plt_taobao_traderate( tid, oid, dp_id, valid_score, role, nick, result,num_iid,  created, rated_nick, item_title, item_price, content, reply)
    SELECT tid,
           oid,
           dp_id,
           valid_score,
