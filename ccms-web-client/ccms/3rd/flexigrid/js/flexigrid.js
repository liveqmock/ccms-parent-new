	...
	$.addFlex = function (t, p) {
		...
		var g = {
						...
						$('thead tr:first th', g.hDiv).each( //add cell
							function () {
								...
								if($(this).attr('chk_cell')){
									...
									var idx = $(this).attr('axis').substr(3);
									if(hasCheckbox) idx --; //在根据表头来遍历添加数据时，因为传过来的数据比现有的数据少了checkbox那项，故索引要-1，才能与传过来的数据相对应
									td.align = this.align;
			...
			setWidth:function(){//宽度自适应 ps:当缩放窗口时宽度自适应还没实现
				...
				return parseInt(gridWidth*cm.width/widthTotal) - 14; //多出的12为padding和border的值，多点是为了预留出滚动条的位置
			},
			pager: 0
		};
		//init divs
		...
		$('thead tr:first th', g.hDiv).each(function () {
			...
			$(this).empty().append(thdiv).removeAttr('width').mousedown(function (e) {
				if(!$(this).attr('chk_cell')) g.dragStart('colMove', e, this); //阻止有checkbox的单元格拖拽
			}).hover(function () {
		...
		if (p.height == 'auto') {//重写自适应高度
			...
			$(g.bDiv).height($(window).height() - totalH);
			$('html').css('overflow-y','hidden');
			$(window).bind('resize',function(){
				$(g.bDiv).height($(this).height() - totalH);
			});
		}else{
